...
function formatText(noti: Notification) {
  const actor =
    noti.actor.handle !== null && noti.actor.handle !== ""
      ? `@${noti.actor.handle}`
      : noti.actor.displayName ?? "Someone";
  if (noti.type === "REPLY_ON_THREAD") {
    return `${actor} commented to your thread`;
  }
  if (noti.type === "LIKE_ON_THREAD") {
    return `${actor} liked on your thread`;
  }
}
...
function NotificationsPage() {
  ...
  const unreadCount = notifications.filter((noti) => !noti.readAt).length;
